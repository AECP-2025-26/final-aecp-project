...
if uploaded_file is not None:
    try:
        ...
        # Check for rows with missing data (too few columns)
        initial_rows = len(df)
        df_cleaned = df.dropna(how='all') # Drop rows where ALL values are NaN (truly empty lines)
        
        if df_cleaned.shape[1] < 5: # Assuming 'year' + 4 required_cols = 5 total columns
             st.error(f"Data loading error: Expected 5 columns but found {df_cleaned.shape[1]}. Please check your CSV file.")
             st.stop()
        ...
        if removed_rows > 0:
             st.warning(f"Removed {removed_rows} potentially malformed/empty row(s) to prevent errors.")
        # ----------------------------------------
